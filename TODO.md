...
- rename remaining plural filenames like decls.ts -> decl.ts (DONE)
...
- also investigate what extensions are supported by cel-go (possibly under cel-go/ext). make the same extensions available in this cel-ts port, placing code under src/extension
