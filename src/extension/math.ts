...
import { ReceiverVarArgMacro, MacroError, type Macro } from "../parser";
import { macroTargetMatchesNamespace } from "./macro";
import { compareValues } from "./util";

type MathConfig = { version: number };
export type MathOption = (config: MathConfig) => void;

export function MathVersion(version: number): MathOption {
  return (config) => {
...
export function Math(...options: MathOption[]): EnvOptions {
  const config: MathConfig = { version: Number.MAX_SAFE_INTEGER };
    ...
    functions.push(
      ...
      new Function(
        ...
        new Overload("math_round_double", [DoubleType], DoubleType, (arg: Value) => {
          if (!(arg instanceof DoubleValue)) return ErrorValue.typeMismatch("double", arg);
          return DoubleValue.of(globalThis.Math.round(arg.value()));
        })
      ),
      new Function(
        "math.trunc",
      ...
      new Function(
        ...
        new Overload("math_isInf_double", [DoubleType], BoolType, (arg: Value) => {
          if (!(arg instanceof DoubleValue)) return ErrorValue.typeMismatch("double", arg);
          return BoolValue.of(arg.value() === Number.POSITIVE_INFINITY || arg.value() === Number.NEGATIVE_INFINITY);
        })
      ),
      new Function(
        "math.isNaN",
      ...
      new Function(
        ...
        new Overload("math_bitShiftLeft_uint_int", [UintType, IntType], UintType, bitShiftLeftUint)
      ),
      new Function(
        "math.bitShiftRight",
        new Overload("math_bitShiftRight_int_int", [IntType, IntType], IntType, bitShiftRightInt),
        new Overload("math_bitShiftRight_uint_int", [UintType, IntType], UintType, bitShiftRightUint)
      )
    );
  }
